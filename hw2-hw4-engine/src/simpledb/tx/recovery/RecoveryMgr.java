...
public class RecoveryMgr {
   ...
   public void checkpoint(List<Integer> activeIds) {
	    bm.flushAll(txnum); 
	    int lsn = NQCheckpoint.writeToLog(lm, activeIds);
	    lm.flush(lsn);
   ...
   private void doRecover() {
	    ...
	    Set<Integer> activeTxs = new HashSet<>();
	    ...
	    while (iter.hasNext()) {
	        ...
	        int txNum = rec.txNumber();
	        if (rec.op() == LogRecord.COMMIT || rec.op() == LogRecord.ROLLBACK) {
	            finishedTxs.add(txNum);
	        } 
	        else if (rec.op() == LogRecord.START) {
	            activeTxs.remove(txNum);
	        } 
	        else if (rec.op() == LogRecord.NQCKPT) {
	            foundCheckpoint = true;
	            NQCheckpoint nqc = (NQCheckpoint) rec;
	            for (int id : nqc.getActiveIdList()) {
	                if (!finishedTxs.contains(id)) {
	                    activeTxs.add(id);
	                }
	            }
	        } 
	        else if (rec.op() == LogRecord.CHECKPOINT) {
	            foundCheckpoint = true;
	        } 
	        else { 
	        ...
	        if (foundCheckpoint && activeTxs.isEmpty()) {
	            return; 
	        }
	    }
	}
